<template>
    <div class="flex items-center justify-center min-h-screen">
        <div class="w-full p-8 bg-white border rounded-lg shadow-lg border-sky-400 md:w-1/2 lg:w-1/2">
            <form @submit.prevent="submit">
                <h1 class="text-2xl font-bold text-center text-gray-800 mb-7">Edit an answer</h1>

                <div>
                    <InputLabel for="question" value="Question" />

                    <textarea id="question" readonly
                        class="w-full p-2 mt-1 border border-gray-300 bg-gray-100 rounded-md min-h-[7em] cursor-not-allowed focus:border-gray-300"
                        v-model="props.question.content" required autofocus autocomplete="content" />
                </div>
                        ...
                        <tbody>
                            <tr class="bg-slate-50">
                                <td class="px-4 py-2 border-b">
                                    <textarea id="answer" class="w-full min-h-[5em] p-2 border border-gray-300 rounded-md"
                                        v-model="form.content" required></textarea>
                                </td>
                                <td class="px-4 py-2 text-center border-b">
                                    <input id="is-correct" type="checkbox" v-model="form.is_correct" class="w-7 h-7" />
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <InputError class="mt-2" :message="form.errors.content || form.errors.is_correct" />
