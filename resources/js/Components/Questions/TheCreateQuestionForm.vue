    ...
    <div class="flex items-center justify-center min-h-screen">
        <div class="w-full p-8 bg-white border rounded-lg shadow-lg border-sky-400 md:w-1/2 lg:w-2/3">
            <form @submit.prevent="submit">
                <h1 class="text-2xl font-bold text-center text-gray-800 mb-7">Create a question</h1>

                <div>
                    <InputLabel for="content" value="Content" />

                    <textarea id="content" class="w-full p-2 mt-1 border border-gray-300 rounded-md min-h-[7em]"
                        v-model="form.content" required autofocus autocomplete="content" />
                ...
                <div class="mt-4">
                    ...
                    <table class="min-w-full mt-1 bg-white border border-gray-300">
                        <thead>
                            <tr>
                                <th class="px-4 py-2 border-b max-w-[15px]">No.</th>
                        ...
                        <tbody>
                            <tr v-for="(answer, index) in form.answers" :key="index"
                                ...
                                <td class="px-4 py-2 border-b">
                                    <textarea :id="'answer-' + index"
                                        ...
                                        v-model="answer.content" required></textarea>
                                </td>
                                <td class="px-4 py-2 text-center border-b">
                                    <input :id="'is-correct-' + index" type="checkbox" v-model="answer.is_correct"
                                        class="w-7 h-7" />
                                </td>
                                <td class="px-4 py-2 text-center border-b">
                ...
                <button @click.prevent="addNewAnswer" class="px-5 py-3 my-5 rounded bg-sky-200">
                    <span><fa-icon icon="fa-icon fa-plus" class="mr-2" /></span> Add Answer
                </button>
